...
SELECT 
    ...
    COUNT(`Nro Prestamo`) as `Cantidad de Créditos otorgados`,
...
SELECT 
    ...
    SUM(CASE WHEN `Fecha Pago` IS NOT NULL AND `vencida?` != 'Sin Fecha de pago' AND `Fecha Pago` != '' THEN `Importe Cuota` ELSE 0 END) as `cantidad cobrada en el mes` 
FROM data_filtrado 
WHERE 
    `Fecha Pago` IS NOT NULL 
    AND Mes IS NOT NULL 
...
sql_query_productos = """
                      ...
                      FROM data_filtrado
                      WHERE Producto IS NOT NULL AND Producto NOT LIKE '%Reestructuración%' AND Producto IS NOT 'ENIAC'
                      GROUP BY Producto
                      """
...
dataframes_por_padron = calcular_dataframes_por_padron(data_filtrado)
